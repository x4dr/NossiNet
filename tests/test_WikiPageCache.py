"""Tests for WikiPage cache persistence across loading methods."""
...
def setup_module(module: ModuleType) -> None:  # noqa: ARG001
    """Set up a dummy wiki structure in /tmp/test_wiki for cache tests."""
...
def teardown_module(module: ModuleType) -> None:  # noqa: ARG001
    """Remove the dummy wiki structure created by setup_module."""
    # Clean up
    import shutil
...
def test_cache_persistence_across_loading_methods() -> None:
    """Verify cached page data persists across load_locate with stem and full path."""
    ...
    clock_match = page4.get_clock("TestClock")
    assert clock_match is not None
    assert clock_match.group("current") == "6"
...
def test_cache_with_and_without_md() -> None:
    """Verify loading a page with and without .md extension returns the same cached instance."""
    WikiPage.page_cache.clear()

    # Load without .md
    page_no_md = WikiPage.load_locate("test", cache=True)
    assert page_no_md is not None
    page_no_md.change_clock("TestClock", -1)
    page_no_md.cacheupdate()

    # Load with .md
    page_with_md = WikiPage.load_locate("test.md", cache=True)
    assert page_with_md is not None
    clock_match = page_with_md.get_clock("TestClock")
    assert clock_match is not None
    assert clock_match.group("current") == "4"
